...
app.post('/api/users', async (req, res) => {
  ...
  try {
    ...
    const query = 'INSERT INTO profiles (name, age, gender, weight, email, password) VALUES (?, ?, ?, ?, ?, ?)';
    const result = await db.query(query, [name, age, gender, weight, email, hashedPassword]);
    res.status(201).json({ id: result.insertId, name, age, gender, weight, email });
  } catch (error) {
...
app.post('/api/meals', async (req, res) => {
  ...
  try {
    const mealResult = await db.query('INSERT INTO meals (name, userId) VALUES (?, ?)', [name, userId]);
    const mealId = mealResult.insertId;

    let totalEnergy = 0, totalProtein = 0, totalFat = 0, totalCarbohydrates = 0, totalFiber = 0;

    for (const ingredient of ingredients) {
      const ingredientDetails = await db.query('SELECT kcal, protein, fat, carbohydrates, fiber FROM food_items WHERE id = ?', [ingredient.foodItemId]);
      ...
      await db.query('INSERT INTO meal_food_items (mealId, foodItemId, weight) VALUES (?, ?, ?)', [mealId, ingredient.foodItemId, ingredient.weight]);
    }

    // Gem eller returnér samlet næringsdata
    await db.query('UPDATE meals SET totalEnergy = ?, totalProtein = ?, totalFat = ?, totalCarbohydrates = ?, totalFiber = ? WHERE id = ?', [totalEnergy, totalProtein, totalFat, totalCarbohydrates, totalFiber, mealId]);

    res.status(201).json({ id: mealId, name, userId, ingredients, nutrition: { totalEnergy, totalProtein, totalFat, totalCarbohydrates, totalFiber } });
  } catch (error) {
